...
(define-empty-tokens plain-tokens
  (LEFT-PAREN COMMA RIGHT-PAREN LEFT-BRACE RIGHT-BRACE
   IN ASSIGN WITH LESS GREATER LESSEQ GREATEREQ NOTEQUAL EQUAL
   DPPLUS DPMINUS
   EOF))

(define-lex-abbrev line-break #\newline)
(define-lex-abbrev identifier-characters (char-complement (char-set "(;,)=:.~?\"%+-{}| \n")))
(define-lex-abbrev identifier-regexp (:: identifier-characters (:* identifier-characters)))
...
(module+ test
  ...
  (define (get-tokens input-port)
    (let ((with-position (lambda () (token-lexer input-port))))
      (let loop ((tokens '()))
  ...
  (test-token "<" 'LESS)
  (test-token ">" 'GREATER)
  (test-token ">=" 'GREATEREQ)
  (test-token "<=" 'LESSEQ)
  (test-token "!=" 'NOTEQUAL)
  (test-token "dP+" 'DPPLUS)
  (test-token "dP-" 'DPMINUS)
